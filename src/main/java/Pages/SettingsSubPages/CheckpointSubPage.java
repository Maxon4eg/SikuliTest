...
public class CheckpointSubPage extends SettingsPage {
    private final Pattern ID = new Pattern(Props.pathForRun("Checkpoint_ident.png"));
     ...
     * <p/>
     * Если в choose zone  поставить НОЛЬ то сразу же жмакнем enter и выберется след. по списку зона
     *
    ...
    public CheckpointSubPage cpEntry(int chooseChannel, int chooseZone, int access) {
        ...
        if (channelEntry == null && entry != null) {// находим необходимые дропдауны
            try {
        ...
        try {
            ...
            for (int j = 1; j < chooseChannel + 1; j++) {
                entry.type(key);
    ...
    public CheckpointSubPage setPassageDetermination(int checkbox) {
